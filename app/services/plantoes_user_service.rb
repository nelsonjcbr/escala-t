...
class PlantoesUserService
  ...
  def get_plantoes_para_check_in
    @plantoes_check_in = Escalamembro.
      ...
      where("membro_id = #{@user_id} and (escaladays.data + turnos.hora_inicio) between NOW() - INTERVAL '10 hours' and NOW() + INTERVAL '10 hours'")
  end
  ...
  def get_ultimos_plantoes
    @ultimos_plantoes = Escalamembro.
      select("escalamembros.*, escaladays.data+turnos.hora_inicio as inicio, escaladays.data+turnos.hora_fim as fim").
      joins(escala: [:escaladay, :turno] ).
