class EscalacmptsController < ApplicationController
  ...
  def index
    @escalacmpts = if params[:search_query].nil?
                     Escalacmpt.order('cmpt desc, equipe_id').page(params[:page]).per(50)
                   else
                     Escalacmpt.search(params[:search_query]).order('cmpt desc, equipe_id').page(params[:page]).per(50)
                   end
  ...
  def create
    ...
    respond_to do |format|
      if @escalacmpt.save
        format.html { redirect_to escalacmpt_url(@escalacmpt), notice: 'Escala gravada com sucesso.' }
